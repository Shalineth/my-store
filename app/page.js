...
const placeOrder = () => {
  ...
  let orderText = "🛍️ NEW ORDER!%0A%0A"
  cart.forEach(i => {
    orderText += i.name + " Size:" + i.size + " x" + i.qty + "%0A"
  })
  orderText += "%0APayment: " + paymentMethod + "%0ATotal: ₱" + total
  
  window.open("https://m.me/EngrCureq?text=" + orderText, '_blank')
            ...
            <div key={p.id} style={{border: "1px solid #ddd", padding: "15px", borderRadius: "8px", textAlign: "center", transition: "0.2s"}}
              ...
              {p.sizes && (
                <select
                  ...
                  {p.sizes.map(s => <option key={s}>Size {s}</option>)}
                </select>
              )}
